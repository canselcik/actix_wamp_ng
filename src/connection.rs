...
use super::args::RpcEndpoint;
use super::args::*;
use super::error::Error;
use super::messages::{Dict, WampError};
use super::pubsub;
use super::pubsub::{Subscription, WampMessage};
use super::{AuthMethod, ErrorKind};
...
impl Message for OpenSession {
    type Result = Result<u64, super::error::Error>;
}
...
{
    type Result = ActorResponse<Self, u64, super::error::Error>;
    ...
    fn handle(
        ...
        use super::messages::{HelloSpec, Role, RoleDesc};
...
where
    W: Sink<ws::Message, Error = ws::ProtocolError> + Unpin + 'static,
{
    type Result = ActorResponse<Self, RpcCallResponse, super::error::Error>;
...
where
    Transport: Sink<ws::Message, Error = ws::ProtocolError>
        ...
        + 'static,
{
    Request(Request<Connection<SplitSink<Transport, ws::Message>>, super::pubsub::Subscribe>),
    PubRequest(Request<Connection<SplitSink<Transport, ws::Message>>, super::pubsub::Publish>),
        ...
        + 'static,
{
    ...
    fn poll_next(
        ...
        let (ret, next_state) = match &mut *self {
            ...
            FromRequest::PubRequest(req) => match req.poll_unpin(cx) {
                Poll::Pending => return Poll::Pending,
                Poll::Ready(Err(e)) => (Poll::Ready(Some(Err(e.into()))), FromRequest::Closed),
                Poll::Ready(Ok(pubreq)) => match pubreq {
                    Ok(_) => (Poll::Pending, FromRequest::Closed),
                    Err(e) => (Poll::Ready(Some(Err(e.into()))), FromRequest::Closed),
                },
            },
        ...
        + 'static,
{
    ...
    fn subscribe(&self, uri: &str) -> Self::Events {
        FromRequest::Request(self.send(super::pubsub::Subscribe {
            topic: Cow::Owned(uri.into()),
        }))
    }

    fn publish(&self, uri: &str, args: Vec<Value>, kw_args: Option<Dict>) {
        let m = super::pubsub::Publish {
            topic: Cow::Owned(uri.into()),
...
impl<Transport> Handler<super::pubsub::Subscribe> for Connection<SplitSink<Transport, ws::Message>>
where
    Transport: Sink<ws::Message, Error = ws::ProtocolError>
        + Stream<Item = Result<ws::Frame, ws::ProtocolError>>
        + 'static,
{
    type Result = ActorResponse<Self, super::pubsub::Subscription, Error>;

    fn handle(&mut self, msg: super::pubsub::Subscribe, _ctx: &mut Self::Context) -> Self::Result {
        let (tx, rx) = oneshot::channel();
...
impl<Transport> Handler<super::pubsub::Unsubscribe>
...
impl<Transport> Handler<super::pubsub::Publish> for Connection<SplitSink<Transport, ws::Message>>
    where
        Transport: Sink<ws::Message, Error = ws::ProtocolError>
        + Stream<Item = Result<ws::Frame, ws::ProtocolError>>
        + 'static,
{
    ...
    fn handle(&mut self, msg: super::pubsub::Publish, _ctx: &mut Self::Context) -> Self::Result {
        let request_id = gen_id();
        let mut d = Dict::default();
        d.insert("acknowledge".to_string(), Value::Bool(true));
        ActorResponse::reply(
            self.send_message(
                &(PUBLISH, request_id, d, msg.topic.as_ref(), msg.args.unwrap_or_default(), msg.kw_args.unwrap_or_default()),
            ),
        )
    }
}
